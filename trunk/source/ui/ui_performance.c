...
static void UI_Performance_GetConfig (void){
	...
	uiPerformance.screenSize.curValue = Cvar_VariableInteger("cl_viewSize");
	...
	if (Cvar_VariableInteger("r_finish"))
	...
	else if (uiPerformance.shadows.curValue > 2)
		uiPerformance.shadows.curValue = 2;

	if (Cvar_VariableInteger("r_caustics"))
		uiPerformance.caustics.curValue = 1;

	if (Cvar_VariableInteger("cl_particles"))
	...
	if (Cvar_VariableInteger("cl_drawShells"))
		uiPerformance.shellEffects.curValue = 1;

	uiPerformance.screenBlends.curValue = Cvar_VariableInteger("cl_viewBlend");
	...
	if (Cvar_VariableInteger("cl_blood"))
...
static void UI_Performance_SetConfig (void){
	...
	Cvar_SetInteger("cl_viewSize", (int)uiPerformance.screenSize.curValue);
	...
	Cvar_SetInteger("cl_markTime", (int)uiPerformance.decals.curValue * 15000);

	Cvar_SetInteger("cl_brassTime", (int)uiPerformance.ejectingBrass.curValue * 2500);

	Cvar_SetInteger("cl_drawShells", (int)uiPerformance.shellEffects.curValue);
...
static void UI_Performance_UpdateConfig (void){
	...
	Q_snprintfz(sizeText, sizeof(sizeText), "%i", (int)uiPerformance.screenSize.curValue);
