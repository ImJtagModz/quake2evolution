...
#define ART_BANNER				"ui/banners/controls_t"
#define ART_TEXT_LOOK			"ui/text/cont_look_text_p1"
#define ART_TEXT_LOOK2			"ui/text/cont_look_text_p2"
#define ART_TEXT_MOVE			"ui/text/cont_move_text_p1"
#define ART_TEXT_MOVE2			"ui/text/cont_move_text_p2"
#define ART_TEXT_SHOOT			"ui/text/cont_shoot_text_p1"
#define ART_TEXT_SHOOT2			"ui/text/cont_shoot_text_p2"
#define ART_TEXT_MISC			"ui/text/cont_misc_text_p1"
#define ART_TEXT_MISC2			"ui/text/cont_misc_text_p2"
#define ART_LOOK				"ui/buttons/controls_look"
#define ART_MOVE				"ui/buttons/controls_move"
#define ART_SHOOT				"ui/buttons/controls_shoot"
#define ART_MISC				"ui/buttons/controls_misc"
...
static void UI_Controls_GetConfig (void){
	...
	uiControls.mouseSpeed.curValue = Cvar_VariableInteger("sensitivity");
	if (uiControls.mouseSpeed.curValue < 1)
		uiControls.mouseSpeed.curValue = 1;
	else if (uiControls.mouseSpeed.curValue > 30)
		uiControls.mouseSpeed.curValue = 30;
	...
	if (Cvar_VariableInteger("cl_freeLook"))
		uiControls.freeLook.curValue = 1;

	if (Cvar_VariableInteger("in_joystick"))
		uiControls.joystick.curValue = 1;
...
static void UI_Controls_SetConfig (void){
	...
	Cvar_SetInteger("sensitivity", (int)uiControls.mouseSpeed.curValue);

	Cvar_SetInteger("m_filter", (int)uiControls.smoothMouse.curValue);
	...
	Cvar_SetInteger("cl_freeLook", (int)uiControls.freeLook.curValue);

	Cvar_SetInteger("in_joystick", (int)uiControls.joystick.curValue);

	Cvar_SetInteger("cl_run", (int)uiControls.alwaysRun.curValue);
}
...
static void UI_Controls_UpdateConfig (void){
	...
	Q_snprintfz(sensitivityText, sizeof(sensitivityText), "%i", (int)uiControls.mouseSpeed.curValue);
	...
	uiControls.joystick.generic.name = uiControlsYesNo[(int)uiControls.joystick.curValue];
...
static void UI_Controls_Ownerdraw (void *self){
	...
	if (item->type == QMTYPE_ACTION){
		...
		if (binding->bind1 == -1)
			...
			Q_strncpyz(name, Key_KeyNumToString(binding->bind1), sizeof(name));
			Q_strupr(name);

			if (binding->bind2 != -1){
				Q_strncpyz(name2, Key_KeyNumToString(binding->bind2), sizeof(name2));
				Q_strupr(name2);

				Q_strncatz(name, " or ", sizeof(name));
				Q_strncatz(name, name2, sizeof(name));
			}
		}
		...
		*(unsigned *)color = *(unsigned *)uiColorWhite;
...
static void UI_Controls_Init (void){
	...
	uiControls.joystick.generic.id				= ID_JOYSTICK;
	uiControls.joystick.generic.type			= QMTYPE_SPINCONTROL;
	uiControls.joystick.generic.flags			= QMF_CENTER_JUSTIFY | QMF_PULSEIFFOCUS | QMF_DROPSHADOW;
	uiControls.joystick.generic.x				= 580;
	uiControls.joystick.generic.y				= 468;
	uiControls.joystick.generic.width			= 198;
	uiControls.joystick.generic.height			= 30;
	uiControls.joystick.generic.callback		= UI_Controls_Callback;
	uiControls.joystick.generic.statusText		= "Enable/Disable joystick input";
	uiControls.joystick.minValue				= 0;
	uiControls.joystick.maxValue				= 1;
	uiControls.joystick.range					= 1;
	...
	UI_AddItem(&uiControls.menu, (void *)&uiControls.joystick);
...
void UI_Controls_Precache (void){
	...
	R_RegisterShaderNoMip(ART_TEXT_LOOK);
	R_RegisterShaderNoMip(ART_TEXT_LOOK2);
	R_RegisterShaderNoMip(ART_TEXT_MOVE);
	R_RegisterShaderNoMip(ART_TEXT_MOVE2);
	R_RegisterShaderNoMip(ART_TEXT_SHOOT);
	R_RegisterShaderNoMip(ART_TEXT_SHOOT2);
	R_RegisterShaderNoMip(ART_TEXT_MISC);
	R_RegisterShaderNoMip(ART_TEXT_MISC2);
	R_RegisterShaderNoMip(ART_LOOK);
	R_RegisterShaderNoMip(ART_MOVE);
	R_RegisterShaderNoMip(ART_SHOOT);
	R_RegisterShaderNoMip(ART_MISC);
}
