...
typedef struct {
	char					cinematics[MAX_MENUCINEMATICS][MAX_QPATH];
	char					cinematicFiles[MAX_MENUCINEMATICS][MAX_QPATH];
...
static void UI_Cinematics_GetCinList (void){
	...
	num = FS_GetFileList("video", NULL, dir, sizeof(dir));
	if (num > MAX_MENUCINEMATICS)
		num = MAX_MENUCINEMATICS;

	for (i = 0, ptr = dir; i < num; i++, ptr += len){
		len = strlen(ptr) + 1;

		// Only copy .cin and .RoQ files, ignore the rest
		Com_FileExtension(ptr, extension, sizeof(extension));
		if (!Q_stricmp(extension, "cin") || !Q_stricmp(extension, "RoQ")){
			Q_strncpyz(uiCinematics.cinematics[count], ptr, sizeof(uiCinematics.cinematics[count]));
			*strchr(uiCinematics.cinematics[count], '.') = 0;
			Q_snprintfz(uiCinematics.cinematicFiles[count], sizeof(uiCinematics.cinematicFiles[count]), "video/%s", ptr);
			count++;
		}
	}
...
static const char *UI_Cinematics_KeyFunc (int key){
	...
	case K_ESCAPE:
	case K_MOUSE2:
		if (uiCinematics.handle)
			UI_StopCinematic(uiCinematics.handle);
...
static void UI_Cinematics_Callback (void *self, int event){
	...
	if (event == QM_CHANGED){
		...
		uiCinematics.handle = UI_PlayCinematic(uiCinematics.cinematicFiles[uiCinematics.cinList.curItem]);
		return;
	}
...
static void UI_Cinematics_Ownerdraw (void *self){
	...
	if (item->id == ID_CINPREVIEW){
		...
		if (uiCinematics.handle)
			UI_DrawCinematic(uiCinematics.handle, x, y, w, h);
...
void UI_Cinematics_Precache (void){
	...
	R_RegisterShaderNoMip(ART_PREVIEWBACK);
}
