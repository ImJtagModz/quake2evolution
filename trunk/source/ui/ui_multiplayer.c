...
#define ART_BACKGROUND			"ui/misc/ui_sub_multiplayer"
#define ART_BANNER				"ui/banners/multiplayer_t"
...
static void UI_AddressBook_SaveServers (void){
	...
	for (i = 0; i < 16; i++){
		Q_snprintfz(name, sizeof(name), "server%i", i+1);
...
static void UI_AddressBook_Init (void){
	...
	memset(&uiAddressBook, 0, sizeof(uiAddressBook_t));
	...
	for (i = 0, y = 128; i < 16; i++, y += 32){
		...
		Q_strncpyz(uiAddressBook.servers[i].buffer, Cvar_VariableString(va("server%i", i+1)), sizeof(uiAddressBook.servers[i].buffer));
	}
...
static void UI_JoinServer_Init (void){
	...
	memset(&uiJoinServer, 0, sizeof(uiJoinServer_t));
...
static void UI_DMOptions_Init (void){
	...
	memset(&uiDMOptions, 0, sizeof(uiDMOptions_t));
...
static void UI_StartServer_GetMapList (void){
	...
	f = fopen(va("%s/%s/maps.lst", Cvar_VariableString("fs_homePath"), Cvar_VariableString("fs_game")), "rb");
	if (!f){
	...
	while (1){
		...
		Q_strncpyz(uiStartServer.maps[uiStartServer.numMaps], tok, sizeof(uiStartServer.maps[uiStartServer.numMaps]));

		tok = Com_ParseExt(&buf, false);
		if (!tok[0])
			continue;	// Invalid

		Q_strncpyz(uiStartServer.names[uiStartServer.numMaps], tok, sizeof(uiStartServer.names[uiStartServer.numMaps]));
...
static void UI_StartServer_Start (void){
	...
	Cvar_SetInteger("deathmatch", !((int)uiStartServer.rules.curValue));
	Cvar_SetInteger("coop", ((int)uiStartServer.rules.curValue));
	Cvar_SetInteger("timelimit", atoi(uiStartServer.timeLimit.buffer));
	Cvar_SetInteger("fraglimit", atoi(uiStartServer.fragLimit.buffer));
	Cvar_SetInteger("maxclients", atoi(uiStartServer.maxClients.buffer));
	Cvar_Set("sv_hostName", uiStartServer.hostName.buffer);

	Q_strncpyz(map, uiStartServer.maps[(int)uiStartServer.mapList.curValue], sizeof(map));
	...
	if (uiStartServer.rules.curValue){
		...
		if (spot)
			Cbuf_ExecuteText(EXEC_APPEND, va("gamemap *%s$%s\n", map, spot));
		else
...
static void UI_StartServer_Callback (void *self, int event){
	...
	if (event == QM_CHANGED){
		...
		case ID_RULES:
			if (uiStartServer.rules.curValue){
				if (atoi(uiStartServer.maxClients.buffer) > 8)
					Q_snprintfz(uiStartServer.maxClients.buffer, sizeof(uiStartServer.maxClients.buffer), "4");
			}
			else {
				if (atoi(uiStartServer.maxClients.buffer) > 256)
					Q_snprintfz(uiStartServer.maxClients.buffer, sizeof(uiStartServer.maxClients.buffer), "256");
			}
			UI_StartServer_Update();
			break;
		case ID_HOSTNAME:
...
static void UI_StartServer_Init (void){
	...
	memset(&uiStartServer, 0, sizeof(uiStartServer_t));
	...
	Q_snprintfz(uiStartServer.timeLimit.buffer, sizeof(uiStartServer.timeLimit.buffer), "%i", Cvar_VariableInteger("timelimit"));
	...
	Q_snprintfz(uiStartServer.fragLimit.buffer, sizeof(uiStartServer.fragLimit.buffer), "%i", Cvar_VariableInteger("fraglimit"));
	...
	if (Cvar_VariableInteger("maxclients") <= 1)
		Q_snprintfz(uiStartServer.maxClients.buffer, sizeof(uiStartServer.maxClients.buffer), "8");
	else
		Q_snprintfz(uiStartServer.maxClients.buffer, sizeof(uiStartServer.maxClients.buffer), "%i", Cvar_VariableInteger("maxclients"));
	...
	Q_strncpyz(uiStartServer.hostName.buffer, Cvar_VariableString("sv_hostName"), sizeof(uiStartServer.hostName.buffer));
