...
static void CIN_ReadInfo (cinematic_t *cin){
	...
	cin->rawWidth = Clamp(cin->vidWidth, 1, cls.glConfig.maxTextureSize);
	cin->rawHeight = Clamp(cin->vidHeight, 1, cls.glConfig.maxTextureSize);
...
cinHandle_t CIN_PlayCinematic (const char *name, int x, int y, int w, int h, int flags){
	...
	if (cin->flags & CIN_SYSTEM){
		...
		UI_SetActiveMenu(UI_CLOSEMENU);	// Close the menu
	}
