...
int CL_ParsePlayerBeam (struct model_s *model)
{
	...
	if (model == clMedia.heatBeamModel)
		VectorSet(offset, 2, 7, -3);
	else if (model == clMedia.monsterHeatBeamModel)
	{
		model = clMedia.heatBeamModel;
...
void CL_ParseTEnt (void)
{
	...
	case TE_BULLET_SPARKS:
		...
		if (type != TE_SPARKS) 
		{
			...
			cnt = rand()&15;
			switch (cnt) {
			case 1:	S_StartSound (pos, 0, 0, clMedia.sfx.ricochet[0], 1, ATTN_NORM, 0);	break;
			case 2:	S_StartSound (pos, 0, 0, clMedia.sfx.ricochet[1], 1, ATTN_NORM, 0);	break;
			case 3:	S_StartSound (pos, 0, 0, clMedia.sfx.ricochet[2], 1, ATTN_NORM, 0);	break;
			default:
				break;
			}
		}
	...
	case TE_SPLASH:			// bullet hitting water
		...
		if (r == SPLASH_SPARKS) 
		{
			r = (rand () % 3);
			switch (r) {
			case 0:		S_StartSound (pos, 0, 0, clMedia.sfx.spark[0], 1, ATTN_STATIC, 0);	break;
			case 1:		S_StartSound (pos, 0, 0, clMedia.sfx.spark[1], 1, ATTN_STATIC, 0);	break;
			default:	S_StartSound (pos, 0, 0, clMedia.sfx.spark[2], 1, ATTN_STATIC, 0);	break;
			}
		}
		break;
	...
	case TE_GRENADE_EXPLOSION_WATER:
		...
		if (type == TE_GRENADE_EXPLOSION_WATER)
			S_StartSound (pos, 0, 0, clMedia.sfx.waterExplo, 1, ATTN_NORM, 0);
		else
			S_StartSound (pos, 0, 0, clMedia.sfx.grenadeExplo, 1, ATTN_NORM, 0);
		break;
	...
	case TE_PLASMA_EXPLOSION:
		...
		ex->frames = 15;
		CL_ExplosionParticles (pos);
		S_StartSound (pos, 0, 0, clMedia.sfx.rocketExplo, 1, ATTN_NORM, 0);
		break;
	...
	case TE_MEDIC_CABLE_ATTACK:
		ent = CL_ParseBeam (clMedia.parasiteSegmentModel);
		break;
	...
	case TE_GRAPPLE_CABLE:
		ent = CL_ParseBeam2 (clMedia.grappleCableModel);
		break;
...
//PGM
	...
	case TE_LIGHTNING:
		ent = CL_ParseLightning (clMedia.lightningModel);
	...
	case TE_HEATBEAM:
		ent = CL_ParsePlayerBeam (clMedia.heatBeamModel);
		break;

	case TE_MONSTER_HEATBEAM:
		ent = CL_ParsePlayerBeam (clMedia.monsterHeatBeamModel);
		break;
...
//		CL_Tracker_Explode (pos);
		S_StartSound (pos, 0, 0, clMedia.sfx.disruptExplo, 1, ATTN_NORM, 0);
		break;
...
// update beams
	...
	// add new entities for the beams
		...
		if (b->model == clMedia.lightningModel)
		{
			model_length = 35.0;
		...
		if ((b->model == clMedia.lightningModel) && (d <= model_length))
		{
...
// update beams
	for (i=0, b=cl_playerbeams ; i< MAX_BEAMS ; i++, b++)
	{
		...
		if(clMedia.heatBeamModel && (b->model == clMedia.heatBeamModel))
		{
...
//PMM
		if(clMedia.heatBeamModel && (b->model == clMedia.heatBeamModel) && (b->entity == cl.playernum+1))
		{
			vec_t len;
...
//				Com_Printf ("%f %f - %f %f %f\n", -pitch, yaw+180.0, b->offset[0], b->offset[1], b->offset[2]);
	...
	// add new entities for the beams
		...
		memset (&ent, 0, sizeof(ent));
		if (b->model == clMedia.heatBeamModel)
		{
			model_length = 32.0;
		}
		else if (b->model == clMedia.lightningModel)
		{
			model_length = 35.0;
			d-= 20.0;  // correction so it doesn't end in middle of tesla
		}
