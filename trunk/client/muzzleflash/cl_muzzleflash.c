...
//
// cl_muzzleflash.c - muzzle flash management
//
...
void CL_ParseMuzzleFlash (void)
{
	...
	// Weapon silenced volume and light radius
	...
	if (weapon & MZ_SILENCED)
	{
		weapon &= ~MZ_SILENCED;
		volume = 0.2f;
		dl->radius = 100 + (frand () * 31);
	}
	else
		dl->radius = 200 + (frand () * 31);
	...
	case MZ_BLASTER:
		...
		S_StartSound (NULL, i, CHAN_WEAPON, shMedia.sfx.mz.cl_sfx_blasterFire, volume, ATTN_NORM, 0);
		break;

    // MZ_BLUEHYPERBLASTER
	case MZ_BLUEHYPERBLASTER:
		Vec3Set (dl->color, 0, 0, 1);
		S_StartSound (NULL, i, CHAN_WEAPON, shMedia.sfx.mz.cl_sfx_hyperBlasterFire, volume, ATTN_NORM, 0);
		break;
	...
	case MZ_MACHINEGUN:
		Vec3Set (dl->color, 1, 1, 0);
		shMedia.sfx.mz.cl_sfx_machineGun[(rand () % 5)];
		break;
