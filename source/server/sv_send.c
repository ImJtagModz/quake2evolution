...
/*
 ...
 =================
*/
static qboolean SV_RateDrop (client_t *cl){
...
void SV_SendClientMessages (void){
	...
	if ((sv.state == SS_DEMO && sv.demoFile) && !paused->integer){
		r = FS_Read(&len, sizeof(len), sv.demoFile);
		if (r != 4){
			SV_DemoCompleted();
			return;
		}
		...
		if (len > MAX_MSGLEN)
			Com_Error(ERR_DROP, "SV_SendClientMessages: len > MAX_MSGLEN");

		r = FS_Read(data, len, sv.demoFile);
		if (r != len){
			SV_DemoCompleted();
			return;
		}
	...
	for (i = 0, cl = svs.clients; i < sv_maxClients->integer; i++, cl++){
		...
		if (sv.state == SS_DEMO || sv.state == SS_CINEMATIC || sv.state == SS_PIC)
			NetChan_Transmit(&cl->netChan, data, len);
