...
static void CDAudio_Eject (void){
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_SET, MCI_SET_DOOR_OPEN, (DWORD)NULL))
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_SET_DOOR_OPEN failed (%i)\n", dwReturn);
}
...
static void CDAudio_CloseDoor (void){
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_SET, MCI_SET_DOOR_CLOSED, (DWORD)NULL))
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_SET_DOOR_CLOSED failed (%i)\n", dwReturn);
}
...
static void CDAudio_GetAudioDiskInfo (void){
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_WAIT, (DWORD)(LPVOID)&mciStatusParms)){
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_STATUS_READY failed (%i)\n", dwReturn);
		return;
	}
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_WAIT, (DWORD)(LPVOID)&mciStatusParms)){
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_STATUS_NUMBER_OF_TRACKS failed (%i)\n", dwReturn);
		return;
	}
...
static void CDAudio_Play2 (int track, qboolean looping){
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_TRACK | MCI_WAIT, (DWORD)(LPVOID)&mciStatusParms)){
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_STATUS failed (%i)\n", dwReturn);
		return;
	}
...
void CDAudio_Stop (void){
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_STOP, 0, (DWORD)NULL))
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_STOP failed (%i)", dwReturn);
...
static void CDAudio_Pause (void){
	...
	mciGenericParms.dwCallback = (DWORD)vid_hWnd;
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_PAUSE, 0, (DWORD)(LPVOID)&mciGenericParms))
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_PAUSE failed (%i)", dwReturn);
...
LONG CDAudio_MessageHandler (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	case MCI_NOTIFY_SUCCESSFUL:
		if (cdAudio.isPlaying){
			...
			if (cdAudio.playLooping){
				...
				if (++cdAudio.loopCounter >= cd_loopCount->integer)
	...
	case MCI_NOTIFY_SUPERSEDED:
		...
		break;
	default:
		Com_DPrintf(S_COLOR_RED "CDAudio: unexpected MM_MCINOTIFY type (%i)\n", wParam);
...
/*
 ...
 =================
*/
static void CDAudio_MixerShutdown (void){
	...
	if (dwResult = mixerClose(cdAudio.hMixer))
		Com_DPrintf(S_COLOR_RED "CDAudio: mixerClose() failed (%i)\n", dwResult);
...
static void CDAudio_MixerInit (void){
	...
	if (dwResult = mixerOpen(&cdAudio.hMixer, 0, 0, 0, MIXER_OBJECTF_MIXER)){
		Com_DPrintf(S_COLOR_RED "CDAudio: mixerOpen() failed (%i)\n", dwResult);
		return;
	}
	...
	if (dwResult = mixerGetLineInfo((HMIXEROBJ)cdAudio.hMixer, &cdAudio.mixerLine, MIXER_GETLINEINFOF_COMPONENTTYPE)){
		Com_DPrintf(S_COLOR_RED "CDAudio: mixerGetLineInfo() failed (%i)\n", dwResult);
		CDAudio_MixerShutdown();
		return;
	}
	...
	if (dwResult = mixerGetLineControls((HMIXEROBJ)cdAudio.hMixer, &cdAudio.mixerLineControl, MIXER_GETLINECONTROLSF_ONEBYTYPE)){
		Com_DPrintf(S_COLOR_RED "CDAudio: mixerGetLineControls() failed (%i)\n", dwResult);
		CDAudio_MixerShutdown();
		return;
	}
}
...
static int CDAudio_GetMixerVolume (void){
	...
	if (dwResult = mixerGetControlDetails((HMIXEROBJ)cdAudio.hMixer, &mxControlDetails, MIXER_GETCONTROLDETAILSF_VALUE)){
		Com_DPrintf(S_COLOR_RED "CDAudio: mixerGetControlDetails() failed (%i)\n", dwResult);
		return 0;
	}
...
static void CDAudio_SetMixerVolume (int volume){
	...
	if (dwResult = mixerSetControlDetails((HMIXEROBJ)cdAudio.hMixer, &mxControlDetails, MIXER_SETCONTROLDETAILSF_VALUE))
		Com_DPrintf(S_COLOR_RED "CDAudio: mixerSetControlDetails() failed (%i)\n", dwResult);
}
...
void CDAudio_Update (void){
	...
	if (cd_noCD->integer != !cdAudio.enabled){
		if (cd_noCD->integer){
			CDAudio_Pause();
			cdAudio.enabled = false;
		}
	...
	if (cd_volume->modified){
		if (cd_volume->integer > 1.0)
			Cvar_SetValue("cd_volume", 1.0);
		else if (cd_volume->integer < 0.0)
			Cvar_SetValue("cd_volume", 0.0);

		CDAudio_SetMixerVolume(cd_volume->value * 255);
...
static qboolean CDAudio_Startup (void){
	...
	if (dwReturn = mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_SHAREABLE, (DWORD)(LPVOID)&mciOpenParms)){
		Com_DPrintf(S_COLOR_RED "CDAudio: MCI_OPEN failed (%i)\n", dwReturn);
		return false;
	}
    ...
    if (dwReturn = mciSendCommand(cdAudio.wDeviceID, MCI_SET, MCI_SET_TIME_FORMAT, (DWORD)(LPVOID)&mciSetParms)){
    	Com_DPrintf(S_COLOR_RED "CDAudio: MCI_SET_TIME_FORMAT failed (%i)\n", dwReturn);
	...
	cdAudio.enabled = !cd_noCD->integer;
...
void CDAudio_Init (void){
	...
	cd_initCDAudio = Cvar_Get("cd_initCDAudio", "1", CVAR_INIT);
	...
	Cmd_AddCommand("cd", CDAudio_CD_f);

	if (!cd_initCDAudio->integer){
		Com_Printf("Skipping CD Audio initialization\n");
		return;
	}
...
void CDAudio_Shutdown (void){
	...
	if (cdAudio.initialized){
		...
		if (dwResult = mciSendCommand(cdAudio.wDeviceID, MCI_CLOSE, MCI_WAIT, (DWORD)NULL))
			Com_DPrintf(S_COLOR_RED "CDAudio: MCI_CLOSE failed (%i)\n", dwResult);
	}
