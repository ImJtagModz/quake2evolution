...
#define ART_MSGBOX				"ui/title_screen/site_load"
...
static void UI_GoToSite_Init (void){
	...
	memset(&uiGoToSite, 0, sizeof(uiGoToSite_t));
...
void UI_GoToSite_Precache (void){
	...
	R_RegisterShaderNoMip(ART_MSGBOX);
}
...
void UI_GoToSite_Menu (const char *webAddress){
	...
	Q_strncpyz(uiGoToSite.webAddress, webAddress, sizeof(uiGoToSite.webAddress));
