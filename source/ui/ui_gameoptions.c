...
static void UI_GameOptions_GetConfig (void){
	...
	uiGameOptions.currentCrosshair = Cvar_VariableInteger("cl_drawCrosshair");
	...
	uiGameOptions.currentColor = Cvar_VariableInteger("cl_crosshairColor");
	...
	Q_snprintfz(uiGameOptions.crosshairSize.buffer, sizeof(uiGameOptions.crosshairSize.buffer), "%i", Cvar_VariableInteger("cl_crosshairSize"));

	if (Cvar_VariableInteger("cl_crosshairNames"))
		uiGameOptions.identifyTarget.curValue = 1;

	Q_snprintfz(uiGameOptions.maxFPS.buffer, sizeof(uiGameOptions.maxFPS.buffer), "%i", Cvar_VariableInteger("cl_maxFPS"));

	if (Cvar_VariableInteger("cl_newHUD"))
		uiGameOptions.enhancedHUD.curValue = 1;

	Q_snprintfz(uiGameOptions.fov.buffer, sizeof(uiGameOptions.fov.buffer), "%i", Cvar_VariableInteger("fov"));

	uiGameOptions.hand.curValue = Cvar_VariableInteger("hand");
	...
	if (Cvar_VariableInteger("cl_allowDownload"))
...
static void UI_GameOptions_SetConfig (void){
	...
	if (atoi(uiGameOptions.maxFPS.buffer) >= 0)
		Cvar_SetInteger("cl_maxFPS", atoi(uiGameOptions.maxFPS.buffer));
	...
	Cvar_SetInteger("hand", (int)uiGameOptions.hand.curValue);
...
static void UI_GameOptions_Ownerdraw (void *self){
	...
	if (self == (void *)&uiGameOptions.crosshair){
		if (uiGameOptions.currentCrosshair >= 1 && uiGameOptions.currentCrosshair <= NUM_CROSSHAIRS){
			...
			*(unsigned *)color = *(unsigned *)colorTable[Clamp(Cvar_VariableInteger("cl_crosshairColor"), 0, 7)];
			color[3] = 255 * Clamp(Cvar_VariableValue("cl_crosshairAlpha"), 0.0, 1.0);
